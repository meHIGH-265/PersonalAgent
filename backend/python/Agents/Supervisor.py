...
class SupervisorAgent(RunnableAgent):
    def __init__(self, worker_agents: list[RunnableAgent] | None = None):
        if not worker_agents:
            ...
            super().__init__(
                llm=get_llm_instance_with_tools(get_base_tools_by_name()),
                name='Agent',
        ...
        agent_names = self.get_agent_names(worker_agents)
        agent_requirements = self.get_agent_requirements(worker_agents)

        system_prompt = (
            'You are the supervisor of a team of agents. '
            f'These are their IDs:{agent_names}\n\n'
        ...
        super().__init__(
            llm=get_llm_instance_with_tools(),
            name='Supervisor',
    ...
    @staticmethod
    def get_agent_names(worker_agents: list[RunnableAgent]) -> str:
        agent_names = ''
    ...
    @staticmethod
    def get_agent_requirements(worker_agents: list[RunnableAgent]) -> str:
    ...
    def parse_answer(supervisor_answer: str) -> dict[str, str]:
        ...
        try:
            ...
            return { AgentIDs.STOP: f'{supervisor_answer}' }
        ...
        for agent_id in parsed_answer.keys():
            if agent_id not in AgentIDs.All:
    ...
    def distribute_tasks(self, supervisor_input_dict: dict[str, str]) -> dict[str, str]:
        if self.solo_mission:
            answer = self.solve_next_task(supervisor_input_dict[AgentIDs.USER])
            return { AgentIDs.STOP: answer }
        ...
        while True:
            if fail_counter > 1:
                ...
                parsed_answer = {AgentIDs.ERROR: error_message}
                break  # Failure break (will stop)
            answer = self.solve_next_task(supervisor_query)
            try:
