...
from src.MultiAgentSystemLogic.Supervisor import SupervisorAgent
from src.Agents.ScriptsForCreatingAgentFiles.UsedWorkers import use_workers


class MultiAgentSystem:
    def __init__(self, agent_ids: list[str] | None = None):
        if agent_ids is None:
            agent_ids = MyAgentIDs.Workers

        # The worker agents included in the multi-agent system
        self.agents: dict[str, Agent] = {agent_id: MyAgentFactory.create_new_agent(agent_id) for agent_id in agent_ids if agent_id in MyAgentIDs.Workers}
    ...
    def task_agent(self, task: str, agent_id: str) -> None:
        ...
        self.agents[agent_id].give_task(task, submit_answer)
    ...
    def handle_special_agent_inbox(self, special_agent_inbox: dict[str, str],
        ...
        if MyAgentIDs.USER in special_agent_inbox:
            user_message = special_agent_inbox[MyAgentIDs.USER]
            special_agent_inbox.pop(MyAgentIDs.USER)
            with condition:
        ...
        if MyAgentIDs.STOP in special_agent_inbox:
            stop_message = special_agent_inbox[MyAgentIDs.STOP]
            with condition:
        ...
        if MyAgentIDs.ERROR in special_agent_inbox:
            error_message = special_agent_inbox[MyAgentIDs.ERROR]
            with condition:
    ...
    def work(self) -> None:
        ...
        while self.is_working:
            ...
            if MyAgentIDs.STOP in special_agent_inbox or MyAgentIDs.ERROR in special_agent_inbox:
                break
    ...
    def add_user_input(self, user_input: str) -> None:
        ...
        with self.supervisor_input_not_empty_condition:
            with self.supervisor_input_lock:
                if MyAgentIDs.USER not in self.supervisor_input:
                    self.supervisor_input[MyAgentIDs.USER] = user_input
                else:
                    self.supervisor_input[MyAgentIDs.USER] += f'\n\n{user_input}'
            self.supervisor_input_not_empty_condition.notify()
