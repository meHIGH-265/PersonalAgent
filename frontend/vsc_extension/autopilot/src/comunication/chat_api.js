...
const sendUserMessage = async (query, host, port) => {
	try {
		const response = await fetch(`http://${backend_host}:${backend_port}/chat/answer_query`, {
			method: 'POST',
...
const clearHistory = async (host, port) => {
	try {
		const response = await fetch(`http://${backend_host}:${backend_port}/chat/clear_history`, {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
